                    ...
                    <tr  class="hover:bg-gray-50 transition-colors">
                      ...
                      <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-green-100 text-green-800">
                        <c:out value="${enrollment.enrollmentDate}" />
                            ...
                            <c:if test="${enrollment.enrollmentStatus == 'ACTIVE' && sessionScope.user.role == 'TRAINER' || sessionScope.user.role == 'MEMBER'}">
                                <td class="px-6 py-4 whitespace-nowrap text-left text-sm font-medium">
                                    <a class="ml-3" href="cancel?enrollmentId=<c:out value="${enrollment.enrollmentId}" />">Cancel</a>
                                </td>
                            </c:if>

                        <c:if test="${enrollment.enrollmentStatus == 'PENDING' && sessionScope.user.role == 'TRAINER'}">
                            <td class="px-6 py-4 whitespace-nowrap text-left text-sm font-medium">
                                <a class="ml-3" href="accept?enrollmentId=<c:out value="${enrollment.enrollmentId}" />">ACCEPT</a>
                            </td>
                        </c:if>
